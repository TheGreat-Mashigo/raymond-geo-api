### STAGE 1: Build ###

FROM maven:3-openjdk-11 AS MAVEN_BUILD
#FROM  openjdk:11 AS MAVEN_BUILD

MAINTAINER Raymond Mashigo

RUN mkdir /build
COPY pom.xml /build/
COPY src /build/src/
WORKDIR /build/
RUN mvn package

### STAGE 2: Run ###

FROM openjdk:11
WORKDIR /app
COPY --from=MAVEN_BUILD /build/target/raymond-shape-api.jar raymond-shape-api.jar
ENV JAVA_OPTS=""
EXPOSE 8080
#ENTRYPOINT exec java $JAVA_OPTS -jar /raymond-shape-api.jar
ENTRYPOINT ["java","-jar","raymond-shape-api.jar"]

#hlogi/spring-boot-docker   latest    74bf22f9ca7c   18 hours ago   682MB
# run the app
#CMD ["/usr/bin/java", "-jar", "raymond-shape-api.jar"]


########################
#FROM maven:3.6.3-jdk-11-slim@sha256:68ce1cd457891f48d1e137c7d6a4493f60843e84c9e2634e3df1d3d5b381d36c AS build
#RUN mkdir /project
#COPY . /project
#WORKDIR /project
#RUN mvn clean package -DskipTests
#
#
#FROM adoptopenjdk/openjdk11:jre-11.0.9.1_1-alpine@sha256:b6ab039066382d39cfc843914ef1fc624aa60e2a16ede433509ccadd6d995b1f
#RUN mkdir /app
#COPY --from=build /project/target/java-application.jar /app/raymond-shape-api.jar
#WORKDIR /app
#CMD "java" "-jar" "raymond-shape-api.jar"